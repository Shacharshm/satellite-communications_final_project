...
def reorganize_models():
    ...
    for idx, folder in enumerate(model_folders, 1):
        ...
        new_path = new_models_dir / f"model_{idx}"
        ...
        # Process contents from old folder to new folder
        for item in os.listdir(old_path):
            s = old_path / item
            if os.path.isdir(s):
                # Copy the first level subfolder as is
                d = new_path / item
                shutil.copytree(s, d, dirs_exist_ok=True)
                
                # Now rename the subfolders inside this folder
                subfolder_idx = 1
                for subitem in os.listdir(d):
                    sub_s = d / subitem
                    if os.path.isdir(sub_s):
                        sub_d = d / f"model_type_{subfolder_idx}"
        ...
        print(f"Processed {folder} -> model_{idx}")
    
    print("\nReorganization complete!")
    print(f"Models have been reorganized from: {old_models_dir}")
